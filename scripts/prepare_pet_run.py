...
TRANSPARENCY_ARTIFACT_RULES = [
    ...
    "Effects are allowed only when they are state-relevant, opaque, clean-edged, compatible with the 3D pet style, fully inside the same frame slot, and physically touching or overlapping the pet silhouette.",
    ...
    "Do not draw detached effects: floating stars, loose sparkles, floating punctuation, floating icons, falling tear drops, separated smoke clouds, loose dust, disconnected fur bits, disconnected outline bits, or stray pixels.",
...
STATE_REQUIREMENTS = {
    ...
    "waving": [
        "Show the greeting through coordinated acting: paw down, paw raised, paw tilted, paw returning, with small head tilt, bright eyes, tiny mouth change, ear perk, and tail wag.",
        "Do not draw wave marks, motion arcs, lines, sparkles, symbols, or floating effects around the paw.",
    ],
    "jumping": [
        "Show the jump through pose and vertical body position: anticipation, lift, airborne peak, descent, settle, with paw lift, ear/tail lag, and slight squash/stretch.",
        "Do not draw ground shadows, contact shadows, drop shadows, oval shadows, landing marks, dust, smears, bounce pads, or motion marks under the pet.",
        "Keep the background outside the pet perfectly flat chroma key with no darker key-colored patches.",
    ],
    "failed": [
        "Show failure through slumped pose, drooping ears/limbs, closed or sad eyes, softened mouth, lowered tail, and lower body position.",
    ...
    "running-right": [
        "Show locomotion through coordinated body, paws/feet, head bob, ear bounce, tail counter-swing, and accessory follow-through.",
    ...
    "running": [
        "Show the pet actively working or processing, as if running a task: focused posture, attentive eyes, small head movement, busy paws, purposeful bobbing, thinking motion, reading/typing-like posture when user-requested, tool/prop motion only if already part of the pet identity, or other non-locomotion activity.",
...
DIGITAL_PET_STYLE = (
    "Codex 3D desktop pet style: compact full-body 3D companion sprite, "
    "soft toy-like chibi proportions, slightly oversized head, chunky readable "
    "silhouette, rounded limbs, simple expressive face, clean studio lighting, "
    "gentle ambient occlusion, and simplified materials that remain readable at "
    "192x208. Fur, fabric, glossy eyes, collars, bows, and small accessories are "
    "allowed when they are part of the character identity, but they must be clean, "
    "consistent, and not overly detailed. Avoid pixel art, thick pixel outlines, "
    "visible stepped edges, flat cel-only shading, painterly illustration, anime "
    "key art, photorealistic animal portrait, vector sticker art, glossy app-icon "
    "hero treatment, complex tiny accessories, and any detail that disappears at "
    "pet size."
)

ANIMATION_ACTING_RULES = [
    "Animate the pet like a small living 3D companion, not a pasted single moving body part.",
    "Coordinate several small body cues across the frame sequence while preserving identity.",
    "Use head tilt/nod/settle when it supports the action.",
    "Use eyes to blink, look, squint, brighten, or soften when it supports the state.",
    "Use mouth changes such as smile, open mouth, relaxed mouth, or pout when appropriate.",
    "Use ears to perk, bounce, fold, or droop subtly.",
    "Use tail to wag, lift, curl, dip, or lag behind body motion when visible.",
    "Use paws/feet to step, lift, tap, land, wave, or brace.",
    "Use body breathing, slight squash/stretch, lean, bounce, or settle; keep it readable, not chaotic.",
    "Move 2-5 features per animation sequence. Do not change every feature at maximum amplitude.",
    "The first and last frames should loop cleanly unless the state is intentionally one-shot.",
]
...
Use this prompt as an authoritative 3D pet sprite-production spec. Do not expand it into painterly character art, anime key art, pixel art, vector mascot art, glossy app icon, photorealistic animal portrait, or marketing artwork.
...
def row_prompt(
    ...
    animation_acting_text = "\n".join(
        f"- {requirement}" for requirement in ANIMATION_ACTING_RULES
    )
    return f"""Create a single horizontal sprite strip for the Codex app digital pet `{args.pet_id}` in the state `{state}`.

Use the attached reference image(s) for pet identity and the attached base pet image as the canonical design. Use the attached layout guide image only for frame count, slot spacing, centering, and safe padding. Translate any high-resolution reference details into the Codex 3D desktop pet style. Do not simply copy the still reference pose. Generate distinct animation poses that create a readable cycle.

Identity lock:
- Do not redesign the pet. Only change pose/action for the `{state}` animation.
- Preserve the exact head shape, ear/horn/limb shape, face design, markings, palette, material style, body proportions, accessory/prop design, and overall silhouette from the canonical base pet.
...
- Keep the rendering like a compact 3D desktop pet: chunky readable silhouette, soft toy-like materials, simple readable lighting, no complex tiny detail.
...
- Preserve the same silhouette, face, proportions, palette, material, accessories, and props across every frame."""
