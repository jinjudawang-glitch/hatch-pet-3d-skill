...
Default pet art should be a compact 3D desktop companion, not pixel art: soft toy-like proportions, slightly oversized head, readable full-body silhouette, rounded limbs, expressive face, clean studio lighting, gentle ambient occlusion, and simple materials that survive downscaling into a `192x208` cell. Fur, fabric, glossy eyes, collars, bows, and small accessories are allowed when they are part of the character identity, but they must be simplified enough to remain readable at pet size.

Do NOT generate pixel art, thick pixel outlines, visible stepped edges, flat cel-only shading, 2D sticker art, painterly illustration, anime key art, photorealistic animal portraits, or glossy app-icon hero art. References that are realistic or detailed should be translated into a consistent 3D pet model with simplified shapes and soft readable materials before row generation.

## 3D Animation Acting

Each animation row should move like a small living pet, not like a single pasted body part. For every action, coordinate several small body cues while preserving identity:

- `head`: tilt, nod, turn, or settle in sync with the action.
- `eyes`: blink, squint, brighten, look sideways, or soften depending on emotion.
- `mouth`: open, smile, relax, or pout when it supports the state.
- `ears`: perk, bounce, fold, or droop subtly.
- `tail`: wag, lift, curl, dip, or lag behind the body motion.
- `paws/feet`: step, lift, tap, land, wave, or brace.
- `body`: breathe, squash/stretch slightly, lean, bounce, or settle.

Motion should be readable but not chaotic. Move 2-5 features per frame sequence; avoid changing every feature at maximum amplitude. The first and last frames should loop cleanly unless the state is intentionally one-shot.
...
- The effect is opaque, clean-edged, 3D-compatible, and uses non-chroma-key colors.
...
- stray pixels, disconnected fur/outline bits, speckle/noise, cropped body parts, overlapping poses, or any pose that crosses into a neighboring frame slot

State-specific guidance:

- `idle`: keep this calm and low-distraction. Use subtle breathing, blink, tiny head/body bob, ear/tail micro-motion, or another quiet persona-preserving motion. Do not show waving, walking, running, jumping, talking, working, reviewing, emotional reactions, large gestures, item interactions, or new props.
- `waving`: show the wave through paw pose plus supporting cues: head tilt, bright eyes, small mouth change, ear perk, and tail wag. Do not draw wave marks, motion arcs, lines, sparkles, or symbols around the paw.
- `jumping`: show vertical motion through body position, paw lift, ear/tail lag, and squash/stretch. Do not draw shadows, dust, landing marks, impact bursts, bounce pads, or floor cues.
- `failed`: show emotion through ear droop, tail drop, sad eyes, mouth shape, body shrink, and lower posture. Tears or tiny attached effects are allowed if they obey the allowed-effects rules; do not use red X marks, floating symbols, detached smoke, detached stars, or separate tear droplets.
- `review`: show focus through lean, blink, eyes, head tilt, paw position, ear attention, or tail stillness. Do not add magnifying glasses, papers, code, UI, punctuation, or symbols unless that prop already exists in the base pet identity.
- `running-right` and `running-left`: show directional locomotion through body, paw/foot steps, head bob, ear bounce, tail counter-swing, and accessory follow-through. Do not draw speed lines, dust clouds, floor shadows, or motion trails.
- `running`: show an active working/in-progress loop, as if the pet is busy with a task. This can be attentive looking, focused paw movement, reading/typing-like posture when user-requested, purposeful bobbing, or a small prop interaction already in the identity. Do not show literal foot-running, jogging, sprinting, treadmill motion, raised knees, long steps, pumping arms, or directional travel.
...
- Explicitly remind the subagent that the prompt's transparency, effects, and 3D acting rules are mandatory: no detached effects, no wave marks for `waving`, no speed lines or dust for directional running rows, no literal foot-running for the non-directional `running` row, and every action should coordinate head, eyes, mouth, ears, tail, paws/feet, and body where appropriate.
...
- coordinated 3D acting, not a single moving pasted part
...
- For action rows, require coordinated acting across multiple features such as head, eyes, mouth, ears, tail, paws/feet, and body.
